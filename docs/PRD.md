...
- **Método de custeio do CMV:** para reduzir a distorção causada por variação cambial entre compras, o CMV é calculado por **custo médio ponderado** (não pelo custo do lote mais recente, que é usado apenas para fins de precificação no RF04/RF06): `custo_médio = Σ(quantidade_do_lote × custo_unitário_do_lote) ÷ quantidade_total_comprada_no_período`, usando os dados já registrados em `purchase_items`. Essa é uma decisão isolada do cálculo contábil (Fase 2) e não afeta o preço de venda praticado no dia a dia (RF04/RF06), que continua usando o custo do lote mais recente.
- **Critério de aceite:** o CMV apurado no período reflete o custo médio ponderado das compras daquele período (ou do estoque consumido), não apenas o custo do último lote registrado.
